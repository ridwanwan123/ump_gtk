    ...
    <div class="container-fluid p-2">
        <div class="card card-outline card-info shadow-sm">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <span class="badge badge-info">TRIWULAN {{ $tw }}</span>
            </div>
                    ...
                    <table class="table table-bordered table-hover table-striped table-sm nowrap">
                        ...
                        <tbody>
                            @foreach ($pegawaiList as $index => $pegawai)
                                <tr>
                                    <td class="text-center nowrap" style="font-size: 0.65rem;">{{ $index + 1 }}</td>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        paginationLinks.forEach(link => {
            link.addEventListener('click', function(e) {
                const formChanged = Array.from(document.querySelectorAll('input[type=number]'))
                    .some(input => input.value !== '' && input.value !== '0');
                if (formChanged) {
                    e.preventDefault();
                    alert('⚠️ Harap simpan absensi halaman ini sebelum pindah halaman!');
                }
            });
        });
    });
